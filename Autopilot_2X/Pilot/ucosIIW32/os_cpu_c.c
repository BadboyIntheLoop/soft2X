...
// RF - poprawka
HANDLE hRomek = NULL;	// zdarzenie "usypiaj¹ce" i "budz¹ce" InterruptThread
// RF - koniec poprawki
...
{   int i;
    ...
    if (OSTCBCur)
	printf("Cur  pTcb=%X  Prio=%d  Stat=%d\n", (unsigned int)OSTCBCur, OSTCBCur->OSTCBPrio, OSTCBCur->OSTCBStat);

    while (pTcb)
    {	printf("Task pTcb=%X  Prio=%d  Stat=%d ", (unsigned int)pTcb, pTcb->OSTCBPrio, pTcb->OSTCBStat);
...
#endif
{
	// RF - poprawka (zmiana sposobu usypiania i budzenia w¹tku InterruptThread
...
{   int i, j;
//WK: Tu by³o przekroczenie indeksu tablicy pTaskTcb[OS_LOWEST_PRIO + 2]
    //for (i=0; i <= OS_LOWEST_PRIO+2; i++) 			//Search thread array for known priority
    for (i=0; i < OS_LOWEST_PRIO+2; i++) 			//Search thread array for known priority
    {   if (pTcb == pTaskTcb[i])				//If found in thread array, ...
//WK: Tu by³o jeszcze gorsze, OSTCBPrioTbl[OS_LOWEST_PRIO + 1]
	//{   for (j=0; j <= OS_LOWEST_PRIO+2; j++)		//... search in task array
	{   for (j=0; j < OS_LOWEST_PRIO+1; j++)		//... search in task array
...
{   char temp[256];
	...
	{   DBGPRINT(0x00000001, "Old Task %d marked for deletion - %X - %X\n", OSPrioCur, hTaskThread[OSPrioCur], OSTCBCur);
        ...
        {   oldIndex = (INT16S)GetThreadIndexForTask(OSTCBCur);
        }
	nextIndex= (INT16S)GetThreadIndexForTask(OSTCBHighRdy);
...
//TODO:WK os_cpu_c tu coœ jest nie tak z zagnie¿d¿eniami
...
{   char temp[256];
    ...
    {   //if (WaitForSingleObject(hInterruptEvent, OS_INTERRUPT_TIMEOUT) == WAIT_TIMEOUT)	//Wait for a timer interrupt event
		...
		// RF - poprawka
		WaitForSingleObject(hRomek, OS_INTERRUPT_TIMEOUT);
		// RF - koniec poprawki
...
{   int i;
	...
	// RF - poprawka (zainicjowanie zdarzenia, typ: manual reset)
	hRomek = CreateEvent(NULL, TRUE, FALSE, "hRomek");
	// RF - koniec poprawki
...
{   int i;
    ...
    {   printf("ERROR: Create thread error %Xh, err=%d\n", pTcb->OSTCBPrio, GetLastError());
