...
public:
	...
	void ahrsMain(const struct Gauge::measurementT* measurement, const struct Gauge::magnetMeasurementT* magnetMeasurement, float airspeed, unsigned int time, int pressureSource, int magEnable, int flightMode, int version, int debugLevel, float groundSpeed, float track_deg, bool gpsGood, bool activeVz);
	void ahrsMainFG(const struct Gauge::measurementT* measurement, float airspeed, unsigned int time, float phi, float theta, float psi, int version, float groundSpeed, float track, bool gpsGood, bool activeVz);
...
private:
	...
	void estVertSpeed(const struct Gauge::measurementT* measurement, struct ahrs_stateT* ahrs_state, bool active);
