...
void Ahrs::AHRS_THETA_OF_DCM(struct ahrs_stateT *state) const
{
    ...
    {
         //WK: unikni�cie ew. dzielenia przez 0
...
void Ahrs::ahrsMain(const struct Gauge::measurementT* measurement, const struct Gauge::magnetMeasurementT* magnetMeasurement, float airspeed, unsigned int time, int pressureSource, int magEnable,int flightMode, int version, int debugMsg, float groundSpeed, float track_deg, bool gpsGood, bool activeVz)
{
    float gyro[3];
    float accel[3];
    float mag[4];
	...
	//GT: sprawdzi� dla danych pomiarowych z czujnika ci�nienia r�nicowego:
	//	  1.)czy potrzebna jest decymacja
	//	  2.)czy dla IAS w��czona jest filtracja, je�li tak to wyliczy� ias tutaj aby pozby� si� op�nienia 
...
//===============================================================================
	...
	estVertSpeed (measurement, ahrsState, activeVz);
...
void Ahrs::ahrsMainFG(const struct Gauge::measurementT* measurement, float airspeed, unsigned int time,  float phi, float theta, float psi, int version, float groundSpeed, float track_deg, bool gpsGood, bool activeVz)
{
    float gyro[3];
    float accel[3];
    float mag[4];
...
void Ahrs::estVertSpeed(const struct Gauge::measurementT* measurement, ahrs_stateT* ahrs_state, bool active)
{
	//< dongnt11 for Estimating the climb rate by integrating the AccZ in NED
	AHRS_DCM_OF_QUAT(ahrs_state);
	float fDcm1 = -ahrs_state->ahrs_dcm02;
	...
	if (active)
	{
		float estimatedG  = accX * accX + accY * accY + accZ * accZ;
		if (estimatedG >= 2.0f)
		{
			ahrs_state->vertSpeedKF.f32Distance = 0.0f;
			ahrs_state->vertSpeedKF.f32Speed = 0.0f;
			ahrs_state->vertSpeedKF.f32BiasAcc = 0.0f;
		}
	}
