...
public:
	...
	void ahrsMain(const struct Gauge::measurementT* measurement, const struct Gauge::magnetMeasurementT* magnetMeasurement, float airspeed, unsigned int time, int pressureSource, int magEnable, int flightMode, int version, int debugLevel, float groundSpeed, float track_deg);
	void ahrsMainFG(const struct Gauge::measurementT* measurement, float airspeed, unsigned int time, float phi, float theta, float psi, int version, float groundSpeed, float track);
...
private:
	...
	void run_kalman( struct ahrs_stateT *state, const float R_axis, const float error, int active) const;
	...
	void ahrsCompute(struct ahrs_stateT* ahrsState, const float* gyro, float* accel, float psiFromHeading , float airspeed, float *mag, int magEnable, int flightMode, int version, float groundspeed);
