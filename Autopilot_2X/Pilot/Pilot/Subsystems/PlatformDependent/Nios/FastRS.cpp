...
*/
FastRS::FastRS(CPU_INT32U portID, int baudRate, int outBufLines)
{
    fastRSControl_t control; 
    _irqFastRSTag = -1;    // Initialization in a case of not executing the 'linkObserver' method
    int i, cnt;
    ...
    if      (baudRate == 460800) divisor =  0;
       ...
       Log.tryAbort ("Critical Error: FastRS_open_1.");
       initFailed = true;
       return;
    }
    ...
    VIETTEL_FASTRS_mWriteReg(FLYEYE_FASTRS_BASE, FAST_RS_DIVISOR_OFFSET, control.word);
    
    // Reseting of an input buffer
    control.word = VIETTEL_FASTRS_mReadReg(FLYEYE_FASTRS_BASE, FAST_RS_RD_CONTROL_OFFSET);

    cnt = control.bits.rx_level;
    for (i=0; i<cnt; i++) 
    {
        	VIETTEL_FASTRS_mReadReg(FLYEYE_FASTRS_BASE, FAST_RS_RD_FIFO_OFFSET);
    }
...
//    Log.bootPrint ("on "); Log.bootPrint (portName); Log.bootPrint (":\t ");
    ...
    {
       Log.tryAbort ("Critical Error: FastRS_open_2.");
       initFailed = true;
       return;
    }
    ...
    {
        Log.tryAbort ("Critical Error: FastRS_open_3.");
        initFailed = true;
        return;
    }

	// Semaphore controlling access to the buffer (receiving)
    if (!_inBufSem.create("FastRS_in"))
    {
        Log.tryAbort ("Critical Error: FastRS_open_4.");
        initFailed = true;
        return;
    }

	// Semaphore controlling access to the cyclic buffer (transmitting)
    if (!_outBufSem.create("FastRS_out"))
    {
        Log.tryAbort ("Critical Error: FastRS_open_5.");
        initFailed = true;
        return;
    }
...
*/
void FastRS::linkObserver()
{
    _irqFastRSTag = registerSubj (this, IRQ_FAST_RS);
}
...
*/
bool FastRS::getLastLine(char* lineReceived)
{
...
void FastRS::taskIn(void* pdata)
{
    ...
    if (initFailed)
    {
        Log.errorPrintf("FastRS_taskIn_1");
    ...
    while(true)
    {
        ...
        if (!checkAspect(f, _irqFastRSTag))
        ...
        for (i=0; i<cnt; i++) {
        		c = VIETTEL_FASTRS_mReadReg(FLYEYE_FASTRS_BASE, FAST_RS_RD_FIFO_OFFSET);
...
*/
bool FastRS::sendLine (const char* lineToSend, bool suppressErrMsg, bool withWaiting, bool withCrc)
{
...
* .
*/
bool FastRS::sendBinary (const unsigned char* buf, int size)
{
	int i;

    for (i=0; i<size; i++) 
	{
			VIETTEL_FASTRS_mWriteReg(FLYEYE_FASTRS_BASE, FAST_RS_WR_FIFO_OFFSET, buf[i]);
	}
    return true;
}
...
*/
void FastRS::taskOut(void* pdata)
{
    INT8U err;
    char *p;
    fastRSControl_t control;
    ...
    while (true)
    {
        ...
        while (lSystem != lUTmp)
        {
            ...
            if (cnt >= (FAST_RS_BUFFER_SIZE - LINESIZE - 2)) {
            ...
            for (p = outLinesBuffer[newLSystem]; *p !=0; p++)
            		VIETTEL_FASTRS_mWriteReg(FLYEYE_FASTRS_BASE, FAST_RS_WR_FIFO_OFFSET, *p);
