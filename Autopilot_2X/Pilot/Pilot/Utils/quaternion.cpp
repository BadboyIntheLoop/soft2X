...
{
    const float l = sqrtf(q2*q2+q3*q3+q4*q4);
    v = Vector3f(q2,q3,q4);
	if (!Numbers::is_zero(l)) {
        v /= l;
		v *= Numbers::wrap_PI(2.0f * atan2f(l, q1));
    }
}
...
void Quaternion::from_axis_angle_fast(const Vector3f &axis, float theta)
{
    ...
    q1 = 1.0f-(sqt2/2.0f)+sqt2*sqt2/24.0f;
    q2 = axis.x * st2;
    q3 = axis.y * st2;
    q4 = axis.z * st2;
}
...
void Quaternion::rotate_fast(const Vector3f &v)
{
    ...
    //"rotation quaternion"
    const float w2 = 1.0f-(sqt2/2.0f)+sqt2*sqt2/24.0f;
