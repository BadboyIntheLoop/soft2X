...
public:
    ...
    // the target waypoint
    virtual int32_t nav_roll_cd(const PStateData& psd) const = 0;
    virtual float nav_roll_rad(const PStateData& psd) const = 0;

    // return the desired lateral acceleration in m/s/s to move towards
    // the target waypoint
    virtual float lateral_acceleration(void) const = 0;

    // note: all centi-degree values returned in Navigation should
    ...
    virtual void update_waypoint(const PStateData& psd, const GpsPosition &prev_WP, const GpsPosition &next_WP, float dist_min = 0.0f) = 0;
    ...
    // nav_roll_cd()) after this function to ask for the new required
    // navigation attitude/steering.
    virtual void update_loiter(const PStateData& psd, const GpsPosition &center_WP, float radius, int8_t loiter_direction) = 0;
    ...
    virtual void update_heading_hold(const PStateData& psd, int32_t navigation_heading_cd) = 0;
    ...
    // attitude/steering.
    virtual void update_level_flight(const PStateData& psd) = 0;

    virtual void update(const PStateData& psd, const GpsPosition& prev_WP, const GpsPosition& next_WP, 
        float dist_min = 0.0f, float radius = 200.0f, int8_t loiter_direction = 1, int32_t navigation_heading_cd = 0) = 0;
