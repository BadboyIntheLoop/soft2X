...
void Vector3<T>::rotate(enum Rotation rotation)
{
	...
	{// no-op; caller should perform custom rotations via matrix multiplication
							return; }
	}
}
...
{
	return (Numbers::is_equal(x, v.x) && Numbers::is_equal(y, v.y) && Numbers::is_equal(z, v.z));
}
...
template Vector3<INT32S> &Vector3<INT32S>::operator +=(const Vector3<INT32S> &v);
//
